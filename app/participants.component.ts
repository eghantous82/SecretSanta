  ...
  template: `
    <ul class="input-list style-1">
      <li *ngFor="let el of participantsService.getParticipants(); let i = index">
        <input [ngClass]="getNameClasses(i)" type="text" placeholder="Name" [(ngModel)]="el.name"
        style="width: 50%; margin-right: 5px" spellcheck="false"/>
        <input [ngClass]="getEmailClasses(i)" type="text" placeholder="Email" [(ngModel)]="el.email"
        style="width: 50%; margin-left: 5px" spellcheck="false"/>
        <a (click)="remove(i)"><img src="/assets/img/deleteParticipant.png" height="40" width="40"
        [ngStyle]="{'cursor': allowRemove() ? 'pointer' : '', 'opacity': allowRemove() ? '1' : '0.4'}"/></a>
